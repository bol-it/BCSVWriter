...
class BCSVWriter
{
    ...
    protected $output_encoding = 'UTF-8'; // Кодировка вывода
    ...
    public function setOutputEncoding($encoding = '')
    {
        ...
        $this->file->rewind();
        if (mb_convert_case($this->output_encoding, MB_CASE_UPPER, "UTF-8") == 'UTF-8') $this->file->fwrite(chr(0xEF).chr(0xBB).chr(0xBF));
        return $this;
    }
    ...
    protected function convertToDefaultEncoding($val) {
		return mb_convert_encoding($val, $this->output_encoding, static::DEFAULT_ENCODING);
	}
     ...
     * @return void
     */
    protected function isMulti($val = null)
    {
        if (!is_null($val)) {
     ...
     * @return void
     */
    protected function maxCount($val = null)
    {
        if (!is_null($val)) {
            if (is_object($val)) {
            ...
            if (is_array($val)) {
                if ($this->isMulti($val)) {
                    foreach ($val as $item_val) {
                        ...
                        if ($this->array_check_of_length) {
                            $maxCount = $this->maxCount($item_val);
                            if ($max_count > $this->max_length) $this->max_length = $max_count;
                        }
                    }
                    if (!$this->array_check_of_length) $this->writeQueue();
                    return $this;
                }
                $this->queue_of_records->enqueue($val);
                if ($this->array_check_of_length) {
                    $max_count = $this->maxCount($val);
                    if ($max_count > $this->max_length) $this->max_length = $max_count;
                }
                else {
                    $this->writeQueue();
    ...
    protected function writeNoQueue()
    {
        foreach ($this->array_records as $record) {
            ...
            $final_record = $this->convertToDefaultEncoding($final_record);
            $result = $this->file->fputcsv($final_record, $this->delimiter, $this->enclosure, $this->escape);
            if (!$result) throw new Exception('Ошибка записи в файл');
        }
    }
    ...
    {
        if (!$this->queue_of_records->isEmpty()) $this->writeQueue();
        $this->file->rewind();
